    ...
    for main_task in meta_to_train:
        if int(main_task) in selected_tasks:
            for tasks in meta_to_train[main_task]:
                ...
                saving_file_weights = 'Models/weights_{}_M{}_{}.npy'.format(dir_data.split('/')[1].split('.')[0],tasks, N_train)
                saving_file_bias = 'Models/bias_{}_M{}_{}.npy'.format(dir_data.split('/')[1].split('.')[0],tasks, N_train)
                if not (os.path.exists(saving_file_weights) and os.path.exists(saving_file_bias)):
                    ...
                    if test_mode:
                        ...
                        for i in range(0, N_task):
                            ...
                            SeMPL_model.train(X_train, Y_train[:, i][:, np.newaxis], lr_opt[i],max_epoch=2000)
                        ...
                        for i in range(0, N_task):
                            ...
                            if i == 0:
                                ...
                                for lr_index, lr_initial in enumerate(lr_range):
                                    ...
                                    model.read_weights(weights, bias)
                                    model.build_train()
                                    model.train(X_train1, Y_train1[:, i][:, np.newaxis], lr_initial,max_epoch=2000)
                            ...
                            SeMPL_model.train(X_train, Y_train[:, i][:, np.newaxis], lr_opt[i],
                                                 max_epoch=2000)
                            weights, bias = SeMPL_model.get_weights()
