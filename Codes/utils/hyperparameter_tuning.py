...
def nn_l1_val(X_train1, Y_train1, X_train2, Y_train2, config):
    """
    Args:
    ...
    model.build_train()
    model.train(X_train1, Y_train1, config['lr'])
    ...
    for i in range(Y_train1.shape[1]):
        abs_error += np.mean(np.abs(Y_pred_val[:, i:i + 1] - Y_train2[:, i:i + 1]))
        rel_error += np.mean(
            np.abs(np.divide(Y_train2[:, i:i + 1] - Y_pred_val[:, i:i + 1], Y_train2[:, i:i + 1]))) * 100
        print('Tuning... Task {}, MAE: {}, MRE: {}'.format(i + 1, abs_error, rel_error))
    abs_error = np.divide(abs_error, Y_train1.shape[1])
    rel_error = np.divide(rel_error, Y_train1.shape[1])

    print(config)
    # print('abs_error:{}, rel_error:{}'.format(abs_error, rel_error))
    print('Time(m): ', (time.time() - start) / 60)
    return abs_error, rel_error

def nn_l1_val_MTL(X_train1, Y_train1, X_train2, Y_train2, config):
    """
    Args:
        X_train1: train input data (2/3 of the whole training data)
        Y_train1: train output data (2/3 of the whole training data)
