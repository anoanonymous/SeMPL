...
if __name__ == '__main__':
    # for dir_data in ['Data/storm-obj2-3tasks.csv', 'Data/imagemagick-4tasks.csv', 'Data/xz-5tasks.csv', 'Data/x264-diff_input-10tasks.csv', 'Data/gcc-diff_input-5tasks.csv', 'Data/spear-10286-6tasks.csv', 'Data/sac_srad_others-5tasks.csv', 'Data/deeparch-SizeReduction-3tasks.csv', 'Data/sqlite-overwritebatch-4tasks.csv']:
    for dir_data in ['Data/nginx-4tasks.csv', 'Data/exastencils-4tasks.csv']:
        ...
        test_mode = False
        show_plot = False
        selected_tasks = list(range(0,11))
        learned_tasks = []
        seed = 2
        N_train = int(1*N)
        LR_max_iter = 1000 # default 100
        LR_learning_rate = 0.00005 # default 0.00005
        deepperf_layer = 1
        deepperf_neuron = 64
        N_test = 0
        ...
        selected_tasks = [t for t in selected_tasks if t < total_tasks]
        # for i, t in enumerate(selected_tasks):
        #     if t > total_tasks:
        #         selected_tasks.remove(t)
        print('Selected tasks: {}'.format(selected_tasks))
        ...
        for group in counter_result.keys():
            ...
            for main_task in selected_tasks:
                if main_task not in learned_tasks:
                    ...
                    print('\nMain task: {}, meta tasks: {}'.format(main_task, tasks))
                    ...
                    for temp_meta in scott_scores:
                        ...
                        temp_best_meta.append(temp_meta.replace('meta', ''))

                    print('Average rankings: {}'.format(scott_scores))

                    # print('The best meta model to pick for {} is meta{}'.format(main_task, temp_best_meta))
                    ...
                    for temp_meta in enumerate(temp_meta_results):
                        # print(temp_meta)
                        best_meta.append(int(temp_meta[1][0].replace('meta[','').replace(']','')))
                    # print(temp_meta_results.keys())

                    print('Final best meta: {}'.format(best_meta))

                    meta_to_train[main_task] = [best_meta]
            ...
            if save_file:
                ...
                with open(saving_best_meta, 'w') as f:  # save the results
                    f.write(json.dumps(meta_to_train))
                print('Results saved to {}'.format(saving_best_meta))

                # if i_main == 0:
                #     with open(saving_best_meta, 'w') as f:  # save the results
                #         f.write('Main{}-meta{}'.format(main_task, meta_to_train[main_task]))
                # else:
                #     with open(saving_best_meta, 'a') as f:  # save the results
                #         f.write('\nMain{}-meta{}'.format(main_task, meta_to_train[main_task]))
